...
4. ✅ ~~Résidu accepté : le 1er saut `http→https` reste en 302 (entrypoint Traefik)~~ — **réglé le 14/07/2026** : « Always Use HTTPS » activé sur Cloudflare (zone proxifiée depuis le 09/07) → `http→https` en **301 servi au edge**, l'origine ne voit plus les requêtes HTTP (vérifié live : `http://vie-publique.sn` → 301, `http://www` → 301).
