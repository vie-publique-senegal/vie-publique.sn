...
<template>
  <UCard>
    ...
    <h2 class="m-4 font-semibold text-gray-900 dark:text-white">Groupe de métiers</h2>
        ...
        <div
          :class="[
            ...
            open
              ...
              : 'hover:bg-primary-50/50 bg-white dark:bg-gray-800 dark:hover:bg-gray-700',
          ]"
        >
          <!-- Droite: Stats -->
          <div class="flex items-center space-x-3">
            <UBadge color="primary" class="text-sm font-medium"> {{ item.total }} députés </UBadge>
          </div>
        </div>

        <!-- Contenu de l'accordéon -->
        <div
          class="border-primary-100 rounded-b-lg border-2 bg-white p-4 dark:border-gray-700 dark:bg-gray-800"
        >
          <ul class="divide-primary-100 space-y-2 divide-y dark:divide-gray-700">
            <li
              v-for="metier in item.metiers"
              :key="metier.nom"
              class="hover:bg-primary-50 flex items-center justify-between rounded p-2 transition-colors"
            >
              <span class="text-primary-800 dark:text-gray-200">{{ metier.nom }}</span>
