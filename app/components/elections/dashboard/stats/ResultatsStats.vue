...
<template>
  <div
    class="relative overflow-hidden rounded-lg bg-white shadow dark:border-gray-800 dark:bg-gray-900"
  >
    <!-- Corps du tableau -->
    <div class="relative max-h-[600px] overflow-auto">
      <div class="sticky top-0 z-20 border-b bg-gray-50 dark:border-gray-800 dark:bg-gray-900">
        ...
        >
          <!-- Title Column -->
          <div
            :class="[type === 'legislative' ? 'col-span-6' : 'col-span-8', 'flex items-center p-4']"
          >
            Classement
          </div>
          ...
          <div
            :class="[
              type === 'legislative' ? 'col-span-3' : 'col-span-4',
              'hover:text-primary-600 flex cursor-pointer items-center justify-end gap-1 p-4 transition-colors',
            ]"
          ...
          <div
            ...
            class="hover:text-primary-600 col-span-3 flex cursor-pointer items-center justify-end gap-1 p-4 transition-colors"
      ...
      >
        <div class="grid min-w-full grid-cols-12 items-center py-2">
          ...
          <div
            :class="[
              type === 'legislative' ? 'col-span-6' : 'col-span-8',
          ...
          >
            ...
            <div class="min-w-0">
              <h3 class="line-clamp-1 text-sm font-bold text-gray-900 dark:text-white">
                {{
                  sortConfig.field === 'voix' && sortConfig.direction === 'desc'
                    ? `${idx + 1} • `
                    : ''
                }}{{ row.acronym || row.name }}
              </h3>
              <p v-if="row.head_of_list" class="hidden truncate text-[10px] text-gray-500 sm:block">
                {{ row.head_of_list.first_name }} {{ row.head_of_list.last_name }}
              </p>
              <p v-else class="hidden truncate text-[10px] text-gray-500 sm:block">
                {{ row.name }}
              </p>
            </div>
          </div>
          ...
          <div :class="[type === 'legislative' ? 'col-span-3' : 'col-span-4', 'px-4 text-right']">
            <div class="flex flex-col items-end">
              ...
              <UBadge
                variant="subtle"
                size="xs"
                :color="row.rawPourcentage > 10 ? 'primary' : 'gray'"
                class="mt-0.5 origin-right scale-90"
              >
