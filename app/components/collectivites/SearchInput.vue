<!--
  Champ de recherche du module : annuaire, hub des départements et page région
  affichent exactement le même, à leur placeholder près. Un seul composant pour
  que l'ajustement d'un état de focus ou d'un contraste ne soit pas à refaire
  page par page (et n'y diverge pas).

  Champ non contrôlé côté DOM (`:value` + `@input`) plutôt que `v-model` natif :
  le parent reste seul maître de l'état, qu'il lit de `route.query` au setup pour
  que le rendu serveur d'un `?q=…` partagé soit déjà filtré.
-->
<template>
...
interface Props {
  modelValue: string;
  /** Ce que la page cherche réellement, énuméré : le placeholder est l'aide. */
  placeholder: string;
  /** Libellé accessible si le placeholder ne suffit pas à nommer le champ. */
  ariaLabel?: string;
}
