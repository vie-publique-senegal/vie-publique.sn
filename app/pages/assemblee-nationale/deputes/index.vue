  ...
  <div class="min-h-screen bg-gray-50 pb-20 dark:bg-gray-950">
    ...
    >
      <div class="container mx-auto px-4 py-3">
        ...
        <div class="flex items-center justify-between">
          <div>
            <h1 class="text-lg font-bold text-gray-900 dark:text-white sm:text-xl">Députés</h1>
            <p class="text-xs text-gray-500 dark:text-gray-400">
              {{ filteredDeputiesCount }} député{{ filteredDeputiesCount > 1 ? 's' : '' }} · 15e
              législature
            </p>
          </div>
        ...
        >
          <div class="flex gap-1.5 py-0.5">
            <button
              ...
              :style="
                filterValue === group.value && group.value !== 'all'
    ...
    <main class="container mx-auto px-4 pt-4">
      ...
      <AppErrorState
        v-else-if="error"
        :error="error"
        title="Liste des députés indisponible"
        message="La liste des députés n'a pas pu être chargée."
        retryable
        @retry="refresh"
      />
      ...
      <div v-else class="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
        ...
        >
          ...
          >
            {{
              deputy.electoral_list.coalition.abbreviation ||
              deputy.electoral_list.coalition.name.slice(0, 10)
            }}
          </div>
          ...
          <div class="absolute inset-x-0 bottom-0 p-3">
            ...
            <p class="mt-0.5 truncate text-[11px] text-white/80">
              ...
              <span v-if="deputy.profession" class="capitalize">{{
                deputy.profession.toLowerCase()
              }}</span>
            </p>
            <p
              v-if="deputy.electoral_list?.constituency"
              class="mt-1 truncate text-[10px] font-medium text-white/70"
            >
  ...
  script: [
    {
      ...
      innerHTML: JSON.stringify(deputiesSchema),
    },
    {
      type: 'application/ld+json',
    ...
    {
      type: 'application/ld+json',
      innerHTML: JSON.stringify(governmentSchema),
    },
  ],
});
...
const { deputies, loading, error, refresh, totalItems, searchQuery, filterValue, setFilterValue } =
  useAssemblyDeputies();
