...
const filteredDetails = computed(() => {
  ...
  if (selectedMunicipality.value) {
    filtered = filtered.filter((item) => item.municipality === selectedMunicipality.value);
  }
...
const localStats = computed(() => {
  ...
  return {
    totalVoters: filteredDetails.value.reduce((sum, item) => sum + item.voters, 0),
    totalOffices: filteredDetails.value.length,
    uniqueMunicipalities: new Set(filteredDetails.value.map((item) => item.municipality)).size,
    uniquePollingPlaces: new Set(filteredDetails.value.map((item) => item.polling_place)).size,
  };
});
    ...
    <template v-else>
      ...
      >
        <div class="text-sm text-gray-600 dark:text-gray-300">
          ...
          bureau(x) de vote,
          <span class="font-medium">{{ localStats.uniqueMunicipalities }}</span>
          commune(s),
