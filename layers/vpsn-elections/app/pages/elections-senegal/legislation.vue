...
const typeOptions = computed(() => {
  ...
  const typeLabels: Record<string, string> = {
    'presidential': 'PrÃ©sidentielle',
    'legislative': 'LÃ©gislatives',
    'locale': 'Locales'
  };
...
const yearOptions = computed(() => {
  if (!config.value?.elections) return [{ label: 'Toutes les annÃ©es', value: 'all' }];
  ...
  return [{ label: 'Toutes les annÃ©es', value: 'all' }, ...options];
});

const sortOptions = [
  { label: 'Plus rÃ©cent', value: '-publish_date' },
  { label: 'Plus ancien', value: 'publish_date' },
  { label: 'Titre A-Z', value: 'title' },
];

useHead({
  title: 'LÃ©gislation Ã‰lectorale | SÃ©nÃ©gal',
  meta: [
    { name: 'description', content: 'Consultez les textes de loi, dÃ©crets et documents officiels rÃ©gissant les Ã©lections au SÃ©nÃ©gal.' }
  ]
});
</script>
  ...
  <div class="min-h-screen bg-slate-50 dark:bg-gray-950 pb-20">
      ...
      <div class="container mx-auto px-4 max-w-6xl">
        ...
        <nav class="mb-6">
          <NuxtLink to="/elections-senegal" class="inline-flex items-center text-sm font-bold text-gray-400 hover:text-primary-600 transition-colors">
            <UIcon name="i-heroicons-arrow-left" class="mr-2 h-4 w-4" /> Retour Ã‰lections
          </NuxtLink>
        </nav>

        <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div class="space-y-1">
            <h1 class="text-2xl font-black uppercase tracking-tight text-gray-900 dark:text-white">LÃ©gislation Ã‰lectorale</h1>
            <p class="text-xs text-gray-500 font-bold uppercase tracking-wider italic">Textes de lois, dÃ©crets et arrÃªtÃ©s officiels</p>
          </div>
    ...
    <div class="container mx-auto px-4 max-w-6xl py-10">
      ...
      <div class="mb-10 flex flex-col md:flex-row gap-4 items-center">
        <div class="relative flex-1 w-full">
           <UInput
            ...
            placeholder="Rechercher un dÃ©cret, une loi..."
            size="lg"
            class="w-full"
      ...
      <div v-else-if="documents.length === 0" class="text-center py-32 bg-white dark:bg-gray-900 rounded-[2rem] border-2 border-dashed border-gray-100 dark:border-gray-800">
         ...
         <h3 class="text-xl font-black text-gray-400 uppercase italic">Aucun document trouvÃ©</h3>
