 ...
 * - elected: "true" pour restreindre aux candidats élus (optionnel)
 *
...
export default defineCachedEventHandler(
  async (event) => {
    ...
    const elected = query.elected === 'true';
    ...
    try {
      ...
      // Restreindre aux élus (statut _neq archived : la prod garde les candidats en draft)
      if (elected) {
        const electedFilter = { is_elected: { _eq: true }, status: { _neq: 'archived' } };
        filter = filter ? { _and: [filter, electedFilter] } : electedFilter;
      }
      ...
      const candidates = await directus.request(readItems('election_candidates', requestOptions));

      // Agréger les données côté serveur
      const professionCounts = candidates.reduce(
        (
          acc: Record<string, { count: number; label: string }>,
          candidate: { profession?: string },
        ) => {
          const rawProfession = candidate.profession
  ...
  },
  {
    ...
    getKey: (event) => {
      const query = getQuery(event);
      return `election-dashboard-stats-professions-${query.coalition || 'all'}-${query.year || 'all'}-${query.type || 'all'}-${query.elected === 'true' ? 'elected' : 'all'}`;
    },
  },
);
